...
class Part:
    ...
    def draw(self, screen, call_size=10, color=BLUE):
        x = self.x * call_size + 1
        y = self.y * call_size + 1
        w = call_size - 2
        h = call_size - 2
        pygame.draw.rect(screen, color, (x, y, w, h))

        if self.is_head:
            pygame.draw.circle(screen, RED, (x + call_size / 2, y + call_size / 2), 2)

            if self.d == 1:
                pygame.draw.circle(screen, RED, (x + call_size / 2, y + call_size / 2 - 2), 1)
            if self.d == 2:
                pygame.draw.circle(screen, RED, (x + call_size / 2 + 2, y + call_size / 2), 1)
            if self.d == 3:
                pygame.draw.circle(screen, RED, (x + call_size / 2, y + call_size / 2 + 2), 1)
            if self.d == 4:
                pygame.draw.circle(screen, RED, (x + call_size / 2 - 2, y + call_size / 2), 1)
...
class EAT:
    ...
    def draw(self, screen, call_size=10, grid=None, color=LIGHT_GRAY):
        if grid is not None:
